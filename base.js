...
const motsAleatoire = mots[Math.floor(Math.random() * mots.length)];
//const valider = document.getElementById('valider');
//const lettre_entrer = document.getElementById('lettre_entrer');
const lettre_trouver = document.getElementById('lettre_trouver');
...
function initReponse(){
    reponse = "" ;
    nombre_lettre.innerHTML = motsAleatoire.length;
    for (var i = 0 ; i < motsAleatoire.length ; i++) {
        reponse += ' _ ' ;
    }
    lettre_trouver.innerHTML = reponse ;
}

initReponse();
/*valider.addEventListener("click", function () {
    for (let i = 0; i < motsAleatoire.length; i++) {
...
function afficherAlphabet() {
    ...
    document.write("<table style=\"margin-top: 25px; background: #232323; width: 100%; height: 200px; display: flex; justify-content: center; flex-wrap: wrap\">") ;
    document.write("<tr style=\'width: 1100px; height: auto; display: flex; flex-wrap: wrap; justify-content: center; margin-top: 20px\'>") ;
    for (var i = 0 ; i < alphabet.length ; i++) {
        document.write("<td style=\" width: 80px; height: 80px; text-align: center\">") ;
        var lettre = alphabet.charAt(i) ;
        var touche = "<button style=\"background: #141414; color: white; font-family: 'Arial Black',sans-serif; border: outset 2px darkgrey; border-radius: 5px; width: 70px; height: 70px\" onClick=\"touchCar(this.value)\">" + lettre + "</button>" ;
...
function touchCar(car)
{
    var touche = '' ;
    for (var i = 0 ; i < motsAleatoire.length ; i++) {
        if (motsAleatoire.charAt(i) !== car ) {
            touche+= car ;

        }
        else {
            touche += reponse.charAt(i) ;

        }
    }
    lettre_trouver.innerHTML = touche ;
    reponse = touche ;
}


console.log(motsAleatoire);